...
import { signInWithGoogle, signInWithGithub, sendMagicLink } from '@/lib/firebase'
  ...
  if (sent) {
    return (
      <div className="w-full max-w-sm text-center">
        ...
        <p className="text-muted-foreground mt-2 text-sm">We sent a magic link to <strong className="text-foreground">{email}</strong>. Click it to create your account.</p>
        <button onClick={() => setSent(false)} className="text-sm text-muted-foreground hover:text-foreground mt-6 transition-colors">
  ...
  return (
    <div className="w-full max-w-sm">
      <div className="text-center mb-8">
        ...
        <p className="text-muted-foreground mt-2 text-sm">Start creating interactive video experiences</p>
      </div>
      ...
      <form className="space-y-4" onSubmit={handleMagicLink}>
        <div>
          ...
          <input
            ...
            className="w-full h-11 rounded-xl border border-border bg-background px-4 text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/50 transition-all"
          />
        </div>
        <div>
        ...
        <Button type="submit" disabled={!!loading || !email.trim()} className="w-full h-11 rounded-xl text-sm font-medium">
          {loading === 'email' ? <Loader2 size={16} className="animate-spin mr-2" /> : null}
          Send magic link
        </Button>
      </form>
      ...
      <p className="text-center text-xs text-muted-foreground mt-6">
        ...
        {' '}and{' '}
        <Link href="/refund" className="text-primary hover:underline">Terms</Link>.
      </p>
